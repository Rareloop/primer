<?php namespace Rareloop\Templating\Balde\Tests;
...
class PatternTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $this->primer = \Rareloop\Primer\Primer::start(__DIR__.'/primer-test', '\Rareloop\Templating\Blade\BladeTemplate');

        /**
         * Listen for when new Template engines are created
         */
        Event::listen('blade.new', function ($twig) {
            // Make sure the cache is disabled otherwise we'll get unreliable test results
            $finder = new Finder();
            $files = $finder->files()->in(__DIR__.'/primer-test/cache');
